...
describe(require('path').basename(__filename, '.e2e.js'), function () {
      ...
      collateralLimit:
        parseFloat(
          ethers.utils.formatEther(
            require('../../deployments/extras.json').GHO_max_collateral_limit
          )
        ) * 10,
      issuanceRatio: 0,
    });
  ...
  it('should set GHO balance to 600', async () => {
    assert.equal(
    ...
    await setTokenBalance({
      wallet,
      balance: 600,
      tokenAddress: require('../../deployments/extras.json').gho_address,
      friendlyWhale: '0xeBe517846d0F36eCEd99C735cbF6131e1fEB775D',
    });
    assert.equal(await getCollateralBalance({ address, symbol: 'GHO' }), 600);
  });

  it('should approve GHO spending for CoreProxy', async () => {
    assert.equal(
      ...
      false,
      'New wallet has not allowed CoreProxy GHO spending'
    );
  ...
  it('should deposit 500 GHO into the system', async () => {
    assert.equal(await getCollateralBalance({ address, symbol: 'GHO' }), 600);
    assert.deepEqual(await getAccountCollateral({ accountId, symbol: 'GHO' }), {
      totalDeposited: 0,
      totalAssigned: 0,
      totalLocked: 0,
    });

    await depositCollateral({ privateKey, symbol: 'GHO', accountId, amount: 500 });

    assert.equal(await getCollateralBalance({ address, symbol: 'GHO' }), 100);
    assert.deepEqual(await getAccountCollateral({ accountId, symbol: 'GHO' }), {
      totalDeposited: 500,
      totalAssigned: 0,
      totalLocked: 0,
  ...
  it('should delegate 500 GHO into the Spartan Council pool', async () => {
    assert.deepEqual(await getAccountCollateral({ accountId, symbol: 'GHO' }), {
      totalDeposited: 500,
      totalAssigned: 0,
      totalLocked: 0,
